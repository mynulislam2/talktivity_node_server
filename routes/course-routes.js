  ...
  try {
    ...
    if (onboardingResult.rows.length > 0) {
      try {
        ...
        console.log(
          "Personalized course generated with",
        ...
        return res.status(400).json({
          success: false,
          error:
            "Failed to generate personalized course. Please complete onboarding and try again.",
        });
      }
    } else {
    ...
    if (!personalizedTopics || personalizedTopics.length === 0) {
      return res.status(400).json({
        success: false,
        error:
          "Personalized topics could not be generated. Please retry after completing onboarding.",
      });
    }
    ...
    if (
      error.code === "23503" &&
      error.constraint === "user_courses_user_id_fkey"
    ) {
      return res.status(404).json({
        success: false,
        error:
          "User not found. Please log out and log back in to refresh your session.",
        code: "USER_NOT_FOUND",
      });
    }
...
router.get("/courses/status", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await checkAndTriggerNextBatchByTime(
      client,
      userId,
      currentDay,
      currentWeek,
    ...
    res.json({
      ...
      data: {
        ...
        today: {
          ...
          listeningQuizAvailable: isListeningQuizAvailable(
            dayType,
            todayProgress
          ),
        },
...
// NOTE: Course speaking start/end APIs removed.
// Speaking completion is updated server-side by the Python agent when the call ends.
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const lifetimeSeconds = parseInt(
        lifetimeResult.rows[0].total_seconds,
        10
      );
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to end user speaking session" });
    } finally {
      if (client) client.release();
    }
  }
...
router.post("/courses/quiz/complete", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await checkAndTriggerNextBatch(
      client,
      userId,
      progress.day_number,
...
router.post(
  "/courses/listening-quiz/complete",
  authenticateToken,
  async (req, res) => {
    let client;
    try {
...
router.get(
  "/courses/speaking/sessions-by-month",
  authenticateToken,
  async (req, res) => {
    let client;
    try {
    ...
    `,
      ...
      res.status(500).json({
        success: false,
        error:
          "Unable to retrieve speaking sessions at this time. Please try again later.",
      });
    } finally {
      if (client) client.release();
    }
  ...
  try {
    ...
    try {
      ...
      if (allMessages.length === 0) {
        console.log(
          "No conversation data available for AI analysis, using fallback"
        );
        aiAnalysis = {
          fluency: 70,
          vocabulary: 80,
          grammar: 72,
          feedback:
            "No conversation data available for this month. Keep practicing to get personalized feedback!",
        };
      } else {
    ...
    res.status(500).json({
      success: false,
      error:
        "Unable to generate monthly report at this time. Please try again later.",
    });
  } finally {
    if (client) client.release();
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const personalizedCourse = await generatePersonalizedCourse(
        onboardingData,
        conversations
      );
    ...
    `,
      ...
      res.status(500).json({
        success: false,
        error:
          "Unable to generate personalized course at this time. Please try again later.",
      });
    } finally {
      if (client) client.release();
    }
...
${strictJsonWarning}`,
    ...
    {
      role: "user",
      content: `Generate a personalized 1-week course based on this user data: ${JSON.stringify(
        contextData
      )}`,
    },
  ];
    ...
    for (let week = 1; week <= 12; week++) {
      for (let day = 1; day <= 7; day++) {
        ...
        const personalizedTopic =
          dayIndex < personalizedTopics.length
            ? personalizedTopics[dayIndex]
            : null;
        ...
        if (progress) {
          ...
          if (day >= 1 && day <= 5) {
            ...
            isCompleted =
              ...
              progress.listening_quiz_completed;
          } else if (day === 6) {
          ...
          if (week === currentWeek && day === currentDay) {
            console.log(
              `Timeline API - isCompleted calculation for Day ${day}:`
            );
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "Next batch personalized course generated with",
          personalizedTopics.length,
          "topics"
        );
      } catch (error) {
        console.error(
          "Error generating next batch personalized course:",
          error
        );
        return res.status(400).json({
          success: false,
          error:
            "Failed to generate next batch personalized course. Please try again.",
        });
      }
      ...
      if (!personalizedTopics || personalizedTopics.length === 0) {
        return res.status(400).json({
          success: false,
          error:
            "Personalized topics could not be generated for next batch. Please try again.",
        });
      }
      ...
      const updateResult = await client.query(
       ...
       WHERE id = $3
       RETURNING *`,
        [JSON.stringify(updatedTopics), nextBatchNumber, currentCourse.id]
      );

      console.log(
        "Next batch topics appended successfully for course:",
        currentCourse.id
      );
      res.status(201).json({
        success: true,
...
async function checkAndTriggerNextBatch(
  client,
  userId,
  currentDay,
  currentWeek
  ...
  try {
    console.log(
      `Checking for batch completion for user ${userId}, week ${currentWeek}, day ${currentDay}`
    );
    ...
    if (
      currentCourse.batch_status &&
      currentCourse.batch_status.action === "generate_next_batch"
    ) {
    ...
    if (!isLastDayOfBatch) {
      console.log(
        `Not the last day of batch. Current day: ${currentDay}, Batch duration: ${BATCH_DURATION_DAYS}`
      );
      return;
    }
    ...
    if (isFinalWeek) {
      console.log(
        `Course completed! User ${userId} has finished all ${TOTAL_COURSE_WEEKS} weeks`
      );
      ...
      console.log(
        `Course marked as completed for user ${userId}, week ${currentWeek}, day ${currentDay}`
      );
      return;
    }
    ...
    console.log(
      `Batch generation triggered by completion for user ${userId}, week ${currentWeek}, day ${currentDay}, batch ${nextBatchNumber}`
    );
  } catch (error) {
...
async function checkAndTriggerNextBatchByTime(
  client,
  userId,
  currentDay,
  currentWeek,
  ...
  try {
    console.log(
      `Checking for batch generation by time for user ${userId}, week ${currentWeek}, day ${currentDay}`
    );

    // Check if this is the first time we're triggering for this week
    if (
      course.batch_status &&
      course.batch_status.action === "generate_next_batch"
    ) {
      console.log("Batch generation already triggered for this week");
      return;
    }
    ...
    if (!isFirstDayOfNewWeek) {
      console.log(
        `Not the first day of a new week. Current day: ${currentDay}, Current week: ${currentWeek}`
      );
      return;
    }
    ...
    await client.query(
      ...
      [
        JSON.stringify({
          ...
          message: `Week ${
            currentWeek - 1
          } completed! Ready to generate batch ${nextBatchNumber} for week ${currentWeek}`,
          batch_number: nextBatchNumber,
    ...
    console.log(
      `Batch generation triggered by time for user ${userId}, week ${currentWeek}, day ${currentDay}, batch ${nextBatchNumber}`
    );
  } catch (error) {
