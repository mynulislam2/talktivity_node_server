...
router.get("/", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      error: "Unable to retrieve groups at this time. Please try again later.",
    });
  }
});

// POST /api/groups/create - Create a new public group
router.post("/create", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      return res
        .status(409)
        .json({ success: false, error: "Group name already exists." });
    }
...
router.post("/:groupId/join", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      error: "Unable to join group at this time. Please try again later.",
    });
  }
});
...
router.post("/:groupId/leave", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      error: "Unable to leave group at this time. Please try again later.",
    });
  }
});
...
router.get("/:groupId/members", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        "Unable to retrieve group members at this time. Please try again later.",
    });
  }
});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        success: false,
        error: "Unable to pin message at this time. Please try again later.",
      });
    }
  }
);
...
router.post("/:groupId/messages/unpin", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      error: "Unable to unpin message at this time. Please try again later.",
    });
  }
});
...
router.post("/:groupId/mute", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      error: "Unable to mute group at this time. Please try again later.",
    });
  }
});
...
router.get("/last-read", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    console.log(
      "✅ Successfully fetched last read status, count:",
      rows.length
    );
    ...
    res.status(500).json({
      success: false,
      error:
        "Unable to retrieve read status at this time. Please try again later.",
    });
  }
});
...
router.delete("/groups/:groupId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      error: "Unable to delete group at this time. Please try again later.",
    });
  }
});
...
router.get("/joined", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, joinedGroups: rows });
  } catch (err) {
    console.error("Error fetching joined groups:", err);
    res
      .status(500)
      .json({ success: false, error: "Unable to retrieve joined groups." });
  }
});
